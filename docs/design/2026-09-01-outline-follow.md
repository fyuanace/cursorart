...
| 2026-09-01 | 视口判定改为：已越过顶沿的最后标题，但顶沿下约 140px 内若有标题则优先它（点击跳转停在顶部下 3–4 行时不再误判上一节）；点大纲后 500ms 内不跟滚动 |
...
- 监听中栏 `.protyle-content` 的捕获阶段 `scroll`：已越过内容顶沿（+8px）的最后一个可用标题；若顶沿下约 140px 内还有标题，优先该标题（官方点击跳转会把目标停在顶部下 3–4 行）。再调用匹配文档的 `outline.setCurrent(heading)`
- 在大纲项 `pointerdown` 后约 500ms 内忽略滚动跟随，避免跳转滚动把官方已选中项改回上一节
...
4. 用方向键在标题间移动：大纲应跟上
5. 点击大纲第三项：正文跳转后高亮应停在第三项，不应弹回第二项
6. 点击大纲项仍应跳转正文；切换主题离开后跟随应停止
7. 分栏打开另一篇文档时，滚动那一篇不应改写当前大纲（root id 不一致）
