...
- 块上带 `custom-fhelper-child-nav` 的引用（fhelper 子文档导航）排除在外：不收集、不注入图标/加粗/下划线，保持官方紫色块引用。原生段落里的同名文档引用仍优化
...
1. 正文插入对另一篇**文档**的引用：名称加粗、正文黑色、链接式下划线，左侧为文档图标；未设自定义图标时应与文件树默认图标同为**线框** SVG，不是实心黑块，也不是旧的蓝色 📄；打开文档时不应先闪紫色
...
6. 带 `custom-fhelper-child-nav` 的标题导航引用：无文档图标、不加粗、无主题下划线，仍为官方紫色；同一篇文档在普通段落里的原生引用仍应有图标与加粗
