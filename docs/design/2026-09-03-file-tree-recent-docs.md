...
| 2026-09-03 | 点击最近打开条目时高亮钉在该项，并去掉官方文件树选中，避免共用滚动区跳到树里 |
| 2026-09-03 | 左侧收藏 / 最近打开 / 文档树同时只保留一处选中：点哪条高亮哪条 |
| 2026-09-03 | 文档删除后，最近打开里对应条目自动去掉（含子文档） |
...
- 点击条目打开文档；高亮钉在刚点的最近打开项上，并去掉官方文件树选中、保持共用滚动区位置。点文档树或顶栏 Tab 时清掉收藏与最近打开的选中，整列只留当前点中的那一条
- 标题栏可折叠；未钉住时当前正文对应项用列表悬停底色标出；条目带官方折叠占位（箭头隐藏）。图标优先同步官方树；未设自定义图标时与文件树 SVG 默认一致，不再回退成旧 📄
- 设置滑杆即时预览显示条数；保存时按新条数裁切已存列表；取消 / Esc / 点遮罩还原未保存条数
- 文档被删除（含删父文档带走的子文档）后，从 `recentDocs` 去掉对应 id 并写回配置。删除事件来自内核主 WebSocket 的 `removeDoc`；启动与树节点消失时再用 `getPathByID` 扫已不存在的条目；打开已删除文档不会再记入
...
1. 开启「加载主题 JS」，reload 后用文件树或顶栏 Tab 切换几篇文档，「最近打开」应按打开顺序更新，当前篇在最前；新文档应出现新条目；未设自定义图标时应与官方树默认图标同形，不是旧的蓝色 📄
2. 再打开已在列表中的文档，该项应立刻顶到第一
3. 点击条目应打开对应文档；该项保持选中底色，官方文件树与收藏都不应再高亮；再点文件树后只留树当前项
4. 设置「样式」关闭「显示最近打开」：区块消失；再打开应恢复。拖动条数只改列出数量；取消后回到保存值
5. 保存后重启，列表与条数仍按主题配置，不依赖官方最近文档对话框
6. 展开/折叠文件树、隐藏笔记本开关，不应拆掉该区块或打乱其缩进
7. 删除一篇在最近打开中的文档（或删除其父文档）：该项从列表消失
